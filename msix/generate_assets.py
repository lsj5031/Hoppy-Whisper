...
def convert_icon_to_png(ico_path: Path, sizes: dict[str, tuple[int, int]], output_dir: Path) -> None:
    """
    ...
    try:
        ...
        if hasattr(img, 'n_frames') and img.n_frames > 1:
            ...
            largest_idx = max(range(len(available_sizes)), key=lambda i: available_sizes[i][0] * available_sizes[i][1])
            img.seek(largest_idx)
            print(f"   Using frame {largest_idx} ({available_sizes[largest_idx][0]}x{available_sizes[largest_idx][1]})")
...
def generate_store_icon(ico_path: Path, output_dir: Path) -> None:
    ...
    try:
        ...
        if hasattr(img, 'n_frames') and img.n_frames > 1:
            ...
            largest_idx = max(range(len(sizes)), key=lambda i: sizes[i][0] * sizes[i][1])
            img.seek(largest_idx)
        ...
        print(f"   ✅ Created: StoreIcon_1024x1024.png")
        print(f"   📤 Upload this to Partner Center for Store listing")
